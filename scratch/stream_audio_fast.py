...
async def stream_audio(file_path: str, role: str):
    ...
    try:
        async with websockets.connect(url) as ws:
            print("Connected successfully. Streaming live audio fast...", flush=True)
            ...
            while offset < total_bytes:
                ...
                # Sleep 20ms to achieve a stable 2.5x speedup without overflowing Scribe's queue
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python stream_audio_fast.py <path_to_wav_file> [role: doctor|patient|attender]", flush=True)
        sys.exit(1)
